...
bool is_in_list(LinkedList *list, GridPosition grid2) {
  ...
  while (aux != NULL) {
    ...
    if (is_grid_equal(cur->grid, grid2))
      return true;
    aux = aux->next;
  }
  return false;
...
GridPosition get_first_move(NodeA *goalNode, GridPosition star) {
  if (goalNode == NULL)
    return star; // n ha caminho

  NodeA *aux = goalNode;

  while (aux->parent != NULL) {
    if (is_grid_equal(aux->parent->grid, star))
      return aux->grid; // chega o proximo
    aux = aux->parent;
  }
...
GridPosition path_finding(TileType grid[GRID_HEIGHT][GRID_WIDTH],
                          GridPosition star, GridPosition goal) {
  ...
  NodeA *starNode = createNodeA(star, 0, manhattan(star, goal),
                                NULL); // g ini 0, vizinho null
  ...
  while (open->head != NULL) { // tenho nos abertos para processar
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (grid[new_row][new_col] != TILE_EMPTY)
        continue;

      GridPosition new_grid_posi = {new_col, new_row};
      if (is_in_list(open, new_grid_posi))
        continue;
      if (is_in_list(closed, new_grid_posi))
        continue;

      NodeA *neighbor = createNodeA(new_grid_posi, current->g + 1,
                                    manhattan(new_grid_posi, goal), current);
